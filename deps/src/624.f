...
C C STOP HERE DAVIDE
C C
C C
C C   THIS DRIVER TESTS SOFTWARE PACKAGES TRIPAK AND SRFPAK
C C FOR TRIANGULATION AND INTERPOLATION ON THE PLANE.  ALL
C C MODULES OTHER THAN TRPLOT AND TRPRNT ARE TESTED UNLESS
C C A TRIANGULATION ERROR IS ENCOUNTERED, IN WHICH CASE THE
C C PROGRAM TERMINATES IMMEDIATELY.  THE INTERPOLATION MODULES
C C ARE TESTED ON DATA FOR WHICH THE METHODS ARE EXACT AND
C C MAXIMUM ERRORS ARE PRINTED.  THESE SHOULD BE CLOSE TO THE
C C MACHINE PRECISION.  BY DEFAULT, TESTS ARE PERFORMED ON A
C C SIMPLE DATA SET CONSISTING OF 13 NODES WHOSE CONVEX HULL
C C COVERS THE UNIT SQUARE.  HOWEVER, BY ENABLING THE READ
C C STATEMENTS BELOW (C* IN THE FIRST TWO COLUMNS), TESTING
C C MAY BE PERFORMED ON AN ARBITRARY SET OF UP TO 100 NODES.
C C NOTE THAT THE CHOICE OF INTERPOLATION/EXTRAPOLATION POINTS
C C ASSUMES THE NODES TO APPROXIMATELY COVER THE UNIT SQUARE.
C C
C C   THE I/O UNITS MAY BE CHANGED BY ALTERING LIN (INPUT) AND
C C LOUT (OUTPUT) IN THE DATA STATEMENT BELOW.  IF NMAX IS
C C INCREASED, DIMENSION STATEMENTS MUST BE CHANGED ACCORDING-
C C LY.
C C
C       INTEGER IADJ(600), IEND(100), NODES(200)
C       REAL    X(100), Y(100), Z(100), ZXZY(2,100), P(5),
C      .        ZZ(5,5)
C       DATA    LIN/5/,  LOUT/6/,  NMAX/100/,  N/13/,
C      .        NROW/5/,  NI/5/,  NJ/5/
C       DATA    X(1),  X(2),  X(3),  X(4),  X(5),  X(6),  X(7)
C      .       / 0.,    1.,    .5,   .35,   .65,   .15,   .5/,
C      .        X(8),  X(9), X(10), X(11), X(12), X(13)
C      .       /.85,   .35,   .65,    .5,    0.,   1./,
C      .        Y(1),  Y(2),  Y(3),  Y(4),  Y(5),  Y(6),  Y(7)
C      .       / 0.,    0.,   .15,   .35,   .35,    .5,   .5/,
C      .        Y(8),  Y(9), Y(10), Y(11), Y(12), Y(13)
C      .       / .5,   .65,   .65,   .85,    1.,   1./
C       DATA    P(1),  P(2),  P(3),  P(4),  P(5)
C      .       /-.1,    .2,    .5,    .8,   1.1/
C       FQ(XF,YF) = ((XF+YF)**2)/4.
C       FL(XF,YF) = (XF+YF)/2.
C C
C C INPUT THE NUMBER OF NODES AND THEIR COORDINATES.
C C
C C*  1 READ (LIN,100) N
C       IF (N .LT. 3  .OR.  N .GT. NMAX) WRITE (LOUT,200) N
C       IF (N .LT. 3  .OR.  N .GT. NMAX) STOP
C C*    READ (LIN,110) (X(I),Y(I), I = 1,N)
C   100 FORMAT (I4)
C   110 FORMAT (F13.8)
C C
C C PRINT A HEADING AND COMPUTE A TOLERANCE FOR TRMTST --
C C   EP10 = 10*EPS WHERE EPS IS THE MACHINE PRECISION.
C C
C       WRITE (LOUT,400) N
C       EP10 = 1.
C     2 EP10 = EP10/2.
C       IF (STORE(EP10+1.) .GT. 1.) GO TO 2
C       EP10 = 20.*EP10
C C
C C TEST REORDR (AND PERMUT) BY SORTING ON X-COMPONENTS AND
C C                          VERIFYING THE ORDERING.
C C
C       CALL REORDR (N,2, X,Y,DUMMY, IEND)
C       DO 3 K = 2,N
C         IF (X(K) .LT. X(K-1)) GO TO 4
C     3   CONTINUE
C       GO TO 5
C     4 WRITE (LOUT,220)
C       STOP
C C
C C TEST TRMESH (REFER TO TRMTST)
C C
C     5 CALL TRMESH (N,X,Y, IADJ,IEND,IER)
C       IF (IER .EQ. 2) WRITE (LOUT,210)
C       IF (IER .EQ. 2) STOP
C       CALL TRMTST (N,X,Y,IADJ,IEND,EP10,LOUT, IER)
C       IF (IER .GT. 0) STOP
C C
C C TEST GETNP BY ORDERING THE NODES ON DISTANCE FROM N0 AND
C C            VERIFYING THE ORDERING.  THE STUFF WITH KSUM IS
C C            TOO CLEVER FOR WORDS.
C C
C       N0 = N/2
C       NODES(1) = N0
C       KSUM = N0
C       DKM1 = 0.
C       DO 6 K = 2,N
C         CALL GETNP (X,Y,IADJ,IEND,K, NODES, DK,IER)
C         IF (IER .NE. 0  .OR.  DK .LT. DKM1) GO TO 7
C         KSUM = KSUM + NODES(K)
C     6   DKM1 = DK
C       IF (KSUM .EQ. (N*(N+1))/2) GO TO 8
C     7 WRITE (LOUT,230)
C       STOP
C C
C C TEST BNODES, AREA, AND VOLUME BY COMPARING THE VALUES
C C              RETURNED BY AREA APPLIED TO THE TRIANGULA-
C C              TION BOUNDARY AND VOLUME APPLIED TO THE
C C              CONSTANT FUNCTION Z = 1.
C C
C     8 CALL BNODES (N,IADJ,IEND, NB,NA,NT,NODES)
C       A1 = AREA(X,Y,NB,NODES)
C       DO 9 K = 1,N
C         Z(K) = 1.
C         ZXZY(1,K) = 0.
C     9   ZXZY(2,K) = 0.
C       A2 = VOLUME(N,X,Y,Z,IADJ,IEND)
C       WRITE (LOUT,300) NB, NT, NA, A1, A2
C C
C C TEST GRADG ON THE CONSTANT FUNCTION.
C C
C       TOL = 0.
C       MAXIT = 1
C       CALL GRADG (N,X,Y,Z,IADJ,IEND,TOL, MAXIT,ZXZY, IER)
C       DMAX = 0.
C       DO 10 K = 1,N
C    10   DMAX = AMAX1(DMAX,ABS(ZXZY(1,K)),ABS(ZXZY(2,K)))
C       WRITE (LOUT,310) DMAX
C C
C C TEST INTRC0 ON THE CONSTANT FUNCTION (FOR WHICH BOTH
C C             INTERPOLATION AND EXTRAPOLATION ARE EXACT).
C C
C       DMAX = 0.
C       IST = 1
C       DO 11 J = 1,NJ
...
C         DO 11 I = 1,NI
C           PX = P(I)
C           CALL INTRC0 (N,PX,PY,X,Y,Z,IADJ,IEND, IST, PZ,
C      .                 IER)
C    11     DMAX = AMAX1(DMAX,ABS(PZ-1.))
C       WRITE (LOUT,320) DMAX
C C
C C TEST GRADL ON A QUADRATIC FUNCTION FQ.  BOTH PARTIAL
C C            DERIVATIVES ARE COMPUTED BY FL.
C C
C       DO 12 K = 1,N
C    12   Z(K) = FQ(X(K),Y(K))
C       DMAX = 0.
...
C         ZX = FL(X(K),Y(K))
C         ZY = ZX
C         CALL GRADL (N,K,X,Y,Z,IADJ,IEND, ZXZY(1,K),
C      .              ZXZY(2,K),IER)
C    13   DMAX = AMAX1(DMAX,ABS(ZX-ZXZY(1,K)),
C      .               ABS(ZY-ZXZY(2,K)))
C       WRITE (LOUT,330) DMAX
C C
C C TEST UNIF (AND INTRC1) ON A LINEAR FUNCTION FL (FOR WHICH
C C                        BOTH INTERPOLATION AND EXTRAPOLA-
C C                        TION ARE EXACT).
C C
C       DO 14 K = 1,N
C    14   Z(K) = FL(X(K),Y(K))
C       CALL UNIF (N,X,Y,Z,IADJ,IEND,NROW,NI,NJ,P,P,2,
C      .           ZXZY, ZZ,IER)
C       DMAX = 0.
C       DO 15 J = 1,NJ
C         DO 15 I = 1,NI
C           ZTRUE = FL(P(I),P(J))
C    15     DMAX = AMAX1(DMAX,ABS(ZTRUE-ZZ(I,J)))
C       WRITE (LOUT,340) DMAX, IER
C C
C C TEST EDGE, USING NODES AS WORK SPACE --
C C            FORCE AN EDGE BETWEEN NODE 1 AND A NODE WHICH
C C            IS NOT ALREADY ADJACENT TO NODE 1 (IF IT
C C            EXISTS).
C C
C       IF (IEND(1) .GE. N-1) WRITE (LOUT,410)
C       IF (IEND(1) .GE. N-1) GO TO 19
C       N1 = 1
C       NP2 = N+2
C       DO 16 K = 2,N
C         N2 = NP2 - K
C         LWK = N
C         CALL EDGE (N1,N2,X,Y, LWK,NODES,IADJ,IEND, IER)
C         IF (IER .NE. 0) GO TO 17
C         IF (LWK .GT. 0) GO TO 18
C    16   CONTINUE
C    17 WRITE (LOUT,240) IER
C       STOP
C C
C C       TEST FOR ALL ARCS (OTHER THAN N1-N2) OPTIMAL.
C C
C    18 L = 1
C       CALL ARCTST (N,X,Y,IADJ,IEND, L, NODES,IER)
C       IF (L .EQ. 1  .AND.  NODES(1) .EQ. N1  .AND.
C      .    NODES(2) .EQ. N2) GO TO 19
C       IF (L .EQ. 0  .AND.  LOPTST(N1,N2,X,Y,IADJ,IEND)
C      .    .EQ. 0) GO TO 19
C       WRITE (LOUT,250) L
C       STOP
C C
C C TEST DELETE BY REMOVING AS MANY BOUNDARY ARCS AS POSSIBLE
C C             AND TESTING THE DATA STRUCTURE FOR ERRORS.
C C
C    19 NDL = 0
C       CALL BNODES (N,IADJ,IEND, NB,NA,NT,NODES)
C       N1 = NODES(NB)
C       DO 20 K = 1,NB
C         N2 = N1
C         N1 = NODES(K)
C         CALL DELETE (N,N1,N2, IADJ,IEND, IER)
C         IF (IER .EQ. 1  .OR.  IER .EQ. 3) GO TO 21
C    20   IF (IER .EQ. 0) NDL = NDL + 1
C       IF (NDL .GT. 0) GO TO 22
C       WRITE (LOUT,420)
C       GO TO 23
C C
C    21 WRITE (LOUT,260) IER
C       STOP
C C
C C       CALL TRMTST WITH THE CIRCUMCIRCLE TEST BYPASSED.
C C
C    22 TOL = 100.
C       CALL TRMTST (N,X,Y,IADJ,IEND,TOL,0, IER)
C       IF (IER .LE. 0) GO TO 23
C       WRITE (LOUT,270) IER
C       STOP
C C
C C SUCCESSFUL TEST
C C
...
C       STOP
C C
C C TRIANGULATION ERROR MESSAGE FORMATS
C C
C   200 FORMAT (//1H ,21HN OUT OF RANGE -- N =,I4)
C   210 FORMAT (1H ,23HALL NODES ARE COLLINEAR)
C   220 FORMAT (1H ,15HERROR IN REORDR)
C   230 FORMAT (1H ,14HERROR IN GETNP)
C   240 FORMAT (1H ,23HERROR IN EDGE -- IER = ,I1)
C   250 FORMAT (1H ,16HERROR IN EDGE --,I3,
C      .        29H ARCS ARE NOT LOCALLY OPTIMAL)
C   260 FORMAT (1H ,30HERROR IN DELETE (INVALID FLAG ,
C      .        19HRETURNED) -- IER = ,I1)
C   270 FORMAT (1H ,31HERROR IN DELETE (TRIANGULATION ,
C      .        9HDESTROYED/1H ,25HERROR FLAG FROM TRMTST = ,
C      .        I1)
C C
C C INTERPOLATION TEST MESSAGE FORMATS
C C
C   300 FORMAT (1H ,36HOUTPUT FROM BNODES, AREA, AND VOLUME/
C      .        1H ,9HBNODES --,I4,18H BOUNDARY NODES,  ,I4,
C      .        13H TRIANGLES,  ,I4,5H ARCS/1H ,
C      .        32HAREA   -- AREA OF CONVEX HULL = ,E15.8/1H ,
C      .        32HVOLUME -- AREA OF CONVEX HULL = ,E15.8//)
C   310 FORMAT (1H ,30HGRADG TEST -- MAXIMUM ERROR = ,
C      .        E15.8//)
C   320 FORMAT (1H ,31HINTRC0 TEST -- MAXIMUM ERROR = ,
C      .        E15.8//)
C   330 FORMAT (1H ,30HGRADL TEST -- MAXIMUM ERROR = ,
C      .        E15.8//)
C   340 FORMAT (1H ,30HUNIF (INTRC1) TEST -- MAXIMUM ,
C      .        8HERROR = ,E15.8/1H ,
C      .        26HEXTRAPOLATION OCCURRED AT ,I2,7H POINTS//)
C C
C C INFORMATIVE MESSAGE FORMATS
C C
C   400 FORMAT (1H1,17X,25HTRIPAK/SRFPAK TEST -- N =,I4///)
C   410 FORMAT (1H ,29HSUBROUTINE EDGE NOT TESTED --/
C      .        1H ,32H  THE LEFTMOST NODE IS ADJACENT ,
C      .        18HTO ALL OTHER NODES//)
C   420 FORMAT (1H ,31HSUBROUTINE DELETE NOT TESTED --/
C      .        1H ,33H  NO BOUNDARY ARC CAN BE REMOVED ,
C      .        7HWITHOUT/
C      .        1H ,30H  DESTROYING THE TRIANGULATION//)
C   430 FORMAT (1H ,35HNO TRIANGULATION ERRORS ENCOUNTERED)
C       END
C       SUBROUTINE ARCTST (N,X,Y,IADJ,IEND, LL, LIST,IER)
C       INTEGER N, IADJ(1), IEND(N), LL, LIST(2,1), IER
C       REAL    X(N), Y(N)
C       LOGICAL SWPTST
C C
C C***********************************************************
C C
C C                                               ROBERT RENKA
C C                                       OAK RIDGE NATL. LAB.
C C                                             (615) 576-5139
C C
C C   THIS ROUTINE DETERMINES THE NUMBER OF ARCS IN A TRIANGU-
C C LATION WHICH ARE NOT LOCALLY OPTIMAL AS DEFINED BY LOGICAL
C C FUNCTION SWPTST.  A THIESSEN TRIANGULATION (SEE SUBROUTINE
C C TRMESH) IS CHARACTERIZED BY THE PROPERTY THAT ALL ARCS ARE
C C LOCALLY OPTIMAL.  THE ALGORITHM CONSISTS OF APPLYING THE
C C SWAP TEST TO ALL INTERIOR ARCS.
C C
C C INPUT PARAMETERS -
C C
C C       N - NUMBER OF NODES IN THE TRIANGULATION.  N .GE. 3.
C C
C C       X,Y - COORDINATES OF THE NODES.
C C
C C       IADJ,IEND - DATA STRUCTURE CONTAINING THE TRIANGULA-
C C                   TION.  SEE SUBROUTINE TRMESH.
C C
C C THE ABOVE PARAMETERS ARE NOT ALTERED BY THIS ROUTINE.
C C
C C       LL - NUMBER OF COLUMNS RESERVED FOR LIST (SEE THE
C C            OUTPUT VALUE OF LIST).  LL .GE. 0.
C C
C C       LIST - INTEGER ARRAY DIMENSIONED 2 BY LL (OR VECTOR
C C              OF LENGTH .GE. 2*LL) IF LL .GT. 0.
C C
C C OUTPUT PARAMETERS -
C C
C C       LL - NUMBER OF ARCS WHICH ARE NOT LOCALLY OPTIMAL
C C            UNLESS IER = 1.
C C
C C       LIST - COLUMNS CONTAIN THE ENDPOINT NODAL INDICES
C C              (SMALLER INDEX IN THE FIRST ROW) OF THE FIRST
C C              K NONOPTIMAL ARCS ENCOUNTERED, WHERE K IS THE
C C              MINIMUM OF THE INPUT AND OUTPUT VALUES OF LL,
C C              IF IER = 0 AND K .GT. 0.  THE NUMBER OF INTE-
C C              RIOR ARCS IS 3N-2NB-3 .LE. 3(N-3) WHERE NB IS
C C              THE NUMBER OF BOUNDARY NODES.  BOUNDARY ARCS
C C              ARE OPTIMAL BY DEFINITION.
C C
C C       IER - ERROR INDICATOR
C C             IER = 0 IF NO ERRORS WERE ENCOUNTERED.
C C             IER = 1 IF N .LT. 3 OR LL .LT. 0.
C C
C C MODULES REQUIRED BY ARCTST - SWPTST
C C
C C***********************************************************
C C
C       NM1 = N - 1
C       LMAX = LL
C C
C C TEST FOR ERRORS AND INITIALIZE FOR LOOP ON INTERIOR ARCS.
C C
C       IF (NM1 .LT. 2  .OR.  LMAX .LT. 0) GO TO 3
C       IER = 0
C       L = 0
C       INDF = 1
C C
C C OUTER LOOP ON NODES IO1
C C
C       DO 2 IO1 = 1,NM1
C         INDL = IEND(IO1)
C         IN2 = IADJ(INDL)
C C
C C INNER LOOP ON NEIGHBORS IO2 OF IO1 SUCH THAT IO2 .GT. IO1
C C   AND IO1-IO2 IS AN INTERIOR ARC -- (IO1,IO2,IN1) AND
C C   (IO2,IO1,IN2) ARE TRIANGLES.
C C
C         DO 1 INDX = INDF,INDL
C           IO2 = IADJ(INDX)
C           IN1 = IADJ(INDX+1)
C           IF (INDX .EQ. INDL) IN1 = IADJ(INDF)
C           IF (IO2 .LT. IO1  .OR.  IN1 .EQ. 0  .OR.
C      .        IN2 .EQ. 0) GO TO 1
C C
C C TEST FOR A SWAP.
C C
C           IF (.NOT. SWPTST(IN1,IN2,IO1,IO2,X,Y)) GO TO 1
C           L = L + 1
C           IF (L .GT. LMAX) GO TO 1
C           LIST(1,L) = IO1
C           LIST(2,L) = IO2
C     1     IN2 = IO2
C     2   INDF = INDL + 1
...
C       RETURN
C C
C C N OR LL OUT OF RANGE
C C
C     3 IER = 1
C       RETURN
C       END
C       SUBROUTINE CIRCUM (X1,X2,X3,Y1,Y2,Y3, CX,CY,IER)
C       INTEGER IER
C       REAL    X1, X2, X3, Y1, Y2, Y3, CX, CY
C C
C C***********************************************************
C C
C C                                               ROBERT RENKA
...
C C                                             (615) 576-5139
C C
C C   THIS SUBROUTINE COMPUTES THE COORDINATES OF THE CENTER
C C OF A CIRCLE DEFINED BY THREE POINTS IN THE PLANE.
C C
C C INPUT PARAMETERS -
C C
C C       X1,...,Y3 - X AND Y COORDINATES OF THREE POINTS IN
C C                   THE PLANE.
C C
C C INPUT PARAMETERS ARE NOT ALTERED BY THIS ROUTINE.
C C
C C OUTPUT PARAMETERS -
C C
C C       CX,CY - COORDINATES OF THE CENTER OF THE CIRCLE
C C               UNLESS IER = 1.
C C
C C       IER - ERROR INDICATOR
C C             IER = 0 IF NO ERRORS WERE ENCOUNTERED.
C C             IER = 1 IF THE POINTS ARE COLLINEAR.
C C
C C MODULES REQUIRED BY CIRCUM - NONE
C C
C C***********************************************************
C C
C       REAL U(3), V(3), DS(3)
C C
C C SET U(K) AND V(K) TO THE X AND Y COMPONENTS OF THE EDGE
C C   OPPOSITE VERTEX K, TREATING THE POINTS AS VERTICES OF
C C   A TRIANGLE.
C C
C       U(1) = X3 - X2
C       U(2) = X1 - X3
...
C       V(3) = Y2 - Y1
C C
C C SET A TO TWICE THE SIGNED AREA OF THE TRIANGLE.  A .GT. 0
C C   IFF (X3,Y3) IS STRICTLY TO THE LEFT OF THE EDGE FROM
C C   (X1,Y1) TO (X2,Y2).
C C
C       A = U(1)*V(2) - U(2)*V(1)
C       IF (A .EQ. 0.) GO TO 2
C C
C C SET DS(K) TO THE SQUARED DISTANCE FROM THE ORIGIN TO
C C   VERTEX K.
C C
C       DS(1) = X1**2 + Y1**2
C       DS(2) = X2**2 + Y2**2
C       DS(3) = X3**2 + Y3**2
C C
C C COMPUTE FACTORS OF CX AND CY.
C C
C       FX = 0.
C       FY = 0.
C       DO 1 I = 1,3
...
C       RETURN
C C
C C COLLINEAR POINTS
C C
C     2 IER = 1
C       RETURN
C       END
C       INTEGER FUNCTION LOPTST (N1,N2,X,Y,IADJ,IEND)
C       INTEGER N1, N2, IADJ(1), IEND(1)
C       REAL    X(1), Y(1)
C C
C C***********************************************************
C C
...
C C                                             (615) 576-5139
C C
C C   GIVEN A PAIR OF INDICES DEFINING A TRIANGULATION ARC,
C C THIS FUNCTION DETERMINES WHETHER OR NOT THE ARC IS LOCALLY
C C OPTIMAL AS DEFINED BY LOGICAL FUNCTION SWPTST.
C C
C C INPUT PARAMETERS -
C C
C C       N1,N2 - X,Y INDICES OF A PAIR OF ADJACENT NODES.
C C
C C       X,Y - NODAL COORDINATES.
C C
C C       IADJ,IEND - DATA STRUCTURE CONTAINING THE TRIANGULA-
C C                   TION.  SEE SUBROUTINE TRMESH.
C C
C C INPUT PARAMETERS ARE NOT ALTERED BY THIS FUNCTION.
C C
C C OUTPUT PARAMETER -
C C
C C       LOPTST = -2 IF N1 AND N2 ARE NOT ADJACENT,
C C              = -1 IF N1-N2 IS AN INTERIOR ARC WHICH IS
C C                   NOT LOCALLY OPTIMAL,
C C              =  0 IF N1-N2 SATISFIES THE NEUTRAL CASE (THE
C C                   VERTICES OF THE CORRESPONDING QUADRILAT-
C C                   ERAL LIE ON A COMMON CIRCLE),
C C              =  1 IF N1-N2 IS A LOCALLY OPTIMAL INTERIOR
C C                   ARC,
C C              =  2 IF N1-N2 IS A BOUNDARY ARC.
C C      NOTE THAT N1-N2 IS LOCALLY OPTIMAL IFF LOPTST .GE. 0.
C C
C C MODULES REQUIRED BY LOPTST - NONE
C C
C C***********************************************************
C C
C       IO1 = N1
C       IO2 = N2
C C
C C FIND THE INDEX OF IO2 AS A NEIGHBOR OF IO1
C C
C       INDF = 1
C       IF (IO1 .GT. 1) INDF = IEND(IO1-1) + 1
C       INDL = IEND(IO1)
C     1 INDX = INDX - 1
C       IF (IADJ(INDX) .EQ. IO2) GO TO 2
C       IF (INDX .NE. INDF) GO TO 1
C C
C C N1 AND N2 ARE NOT ADJACENT
C C
C       LOPTST = -2
C       RETURN
C C
C C DETERMINE IN1 AND IN2 SUCH THAT (IO1,IO2,IN1) AND
C C   (IO2,IO1,IN2) ARE TRIANGLES.
C C
C     2 IF (INDX .NE. INDL) IN1 = IADJ(INDX+1)
C       IF (INDX .EQ. INDL) IN1 = IADJ(INDF)
C       IF (INDX .NE. INDF) IN2 = IADJ(INDX-1)
C       IF (INDX .EQ. INDF) IN2 = IADJ(INDL)
C       IF (IN1 .NE. 0  .AND.  IN2 .NE. 0) GO TO 3
C C
C C N1-N2 IS A BOUNDARY ARC
C C
C       LOPTST = 2
C       RETURN
C C
C C COMPUTE COMPONENTS OF THE QUADRILATERAL SIDES.
C C
C     3 DX11 = X(IO1) - X(IN1)
C       DX12 = X(IO2) - X(IN1)
C       DX22 = X(IO2) - X(IN2)
C       DX21 = X(IO1) - X(IN2)
C C
...
C       DY21 = Y(IO1) - Y(IN2)
C C
C C COMPUTE INNER PRODUCTS.
C C
C       COS1 = DX11*DX12 + DY11*DY12
C       COS2 = DX22*DX21 + DY22*DY21
C C
C C IO1-IO2 IS LOCALLY OPTIMAL IFF A1+A2 .LE. 180 DEGREES
C C   WHERE A1 AND A2 DENOTE THE ANGLES AT IN1 AND IN2.
C C
C       IF (COS1 .LT. 0.  .AND.  COS2 .LT. 0.) GO TO 4
C       IF (COS1 .GT. 0.  .AND.  COS2 .GT. 0.) GO TO 5
C C
C C COMPUTE A QUANTITY WITH THE SIGN OF SIN(A1+A2).
C C
C       SIN12 = (DX11*DY12 - DX12*DY11)*COS2 +
C      .        (DX22*DY21 - DX21*DY22)*COS1
C       IF (SIN12 .LT. 0.) GO TO 4
C       IF (SIN12 .GT. 0.) GO TO 5
C C
C C NEUTRAL CASE
C C
C       LOPTST = 0
C       RETURN
C C
C C N1-N2 NOT LOCALLY OPTIMAL
C C
C     4 LOPTST = -1
C       RETURN
C C
C C N1-N2 LOCALLY OPTIMAL
C C
C     5 LOPTST = 1
C       RETURN
C       END
...
C C                                             (615) 576-5139
C C
C C   THIS FUNCTION FORCES ITS ARGMENT X TO BE STORED IN MAIN
C C MEMORY.  THIS IS USEFUL FOR COMPUTING MACHINE DEPENDENT
C C PARAMETERS (SUCH AS THE MACHINE PRECISION) WHERE IT IS
C C NECESSARY TO AVOID COMPUTATION IN HIGH PRECISION REG-
C C ISTERS.
C C
C C INPUT PARAMETER -
C C
C C       X - VALUE TO BE STORED.
C C
C C X IS NOT ALTERED BY THIS FUNCTION.
C C
C C OUTPUT PARAMETER -
C C
C C       STORE - VALUE OF X AFTER IT HAS BEEN STORED AND
C C               (POSSIBLY) TRUNCATED OR ROUNDED TO THE
C C               MACHINE WORD LENGTH.
C C
C C MODULES REQUIRED BY STORE - NONE
C C
C C***********************************************************
C C
C       COMMON/STCOM/Y
...
C       RETURN
C       END
C       SUBROUTINE TRMTST (N,X,Y,IADJ,IEND,TOL,LUN, IER)
C       INTEGER N, IADJ(1), IEND(N), LUN, IER
C       REAL    X(N), Y(N), TOL
C C
C C***********************************************************
C C
...
C C                                             (615) 576-5139
C C
C C   THIS ROUTINE TESTS THE VALIDITY OF THE DATA STRUCTURE
C C REPRESENTING A THIESSEN TRIANGULATION CREATED BY SUBROU-
C C TINE TRMESH.  THE FOLLOWING PROPERTIES ARE TESTED --
C C   1)  IEND(1) .GE. 3 AND IEND(K) .GE. IEND(K-1)+3 FOR K =
C C       2,...,N (EACH NODE HAS AT LEAST THREE NEIGHBORS).
C C   2)  0 .LE. IADJ(K) .LE. N FOR K = 1,...,IEND(N) (IADJ
C C       ENTRIES ARE NODAL INDICES OR ZEROS REPRESENTING THE
C C       BOUNDARY).
C C   3)  NB .GE. 3, NT = 2N-NB-2, AND NA = 3N-NB-3 WHERE NB,
C C       NT, AND NA ARE THE NUMBERS OF BOUNDARY NODES, TRI-
C C       ANGLES, AND ARCS, RESPECTIVELY.
C C   4)  EACH CIRCUMCIRCLE DEFINED BY THE VERTICES OF A TRI-
C C       ANGLE CONTAINS NO NODES IN ITS INTERIOR.  THIS PROP-
C C       ERTY DISTINGUISHES A THIESSEN TRIANGULATION FROM AN
C C       ARBITRARY TRIANGULATION OF THE NODES.
C C NOTE THAT NO TEST IS MADE FOR THE PROPERTY THAT A TRIANGU-
C C LATION COVERS THE CONVEX HULL OF THE NODES, AND THUS A
C C TEST ON A DATA STRUCTURE ALTERED BY SUBROUTINE DELETE
C C SHOULD NOT RESULT IN AN ERROR.
C C
C C INPUT PARAMETERS -
C C
C C       N - NUMBER OF NODES.  N .GE. 3.
C C
C C       X,Y - NODAL COORDINATES.
C C
C C       IADJ,IEND - TRIANGULATION DATA STRUCTURE.  SEE SUB-
C C                   ROUTINE TRMESH.
C C
C C       TOL - NONNEGATIVE TOLERANCE TO ALLOW FOR FLOATING-
C C             POINT ERRORS IN THE CIRCUMCIRCLE TEST.  AN
C C             ERROR SITUATION IS DEFINED AS R**2 - D**2 .GT.
C C             TOL WHERE R IS THE RADIUS OF A CIRCUMCIRCLE
C C             AND D IS THE DISTANCE FROM THE CIRCUMCENTER
C C             TO THE NEAREST NODE.  A REASONABLE VALUE
C C             FOR TOL IS 10*EPS WHERE EPS IS THE MACHINE
C C             PRECISION.  THE TEST IS EFFECTIVELY BYPASSED
C C             BY MAKING TOL LARGER THAN THE DIAMETER OF THE
C C             CONVEX HULL OF THE NODES.
C C
C C       LUN - LOGICAL UNIT NUMBER FOR PRINTING ERROR MES-
C C             SAGES.  IF LUN .LT. 1 OR LUN .GT. 99, NO MES-
C C             SAGES ARE PRINTED.
C C
C C INPUT PARAMETERS ARE NOT ALTERED BY THIS ROUTINE.
C C
C C OUTPUT PARAMETER -
C C
C C       IER - ERROR INDICATOR
C C             IER = -1 IF ONE OR MORE NULL TRIANGLES (AREA =
C C                      0) ARE PRESENT BUT NO (OTHER) ERRORS
C C                      WERE ENCOUNTERED.  A NULL TRIANGLE IS
C C                      AN ERROR ONLY IF IT OCCURS IN THE
C C                      THE INTERIOR.
C C             IER = 0 IF NO ERRORS OR NULL TRIANGLES WERE
C C                     ENCOUNTERED.
C C             IER = 1 IF N .LT. 3 OR TOL .LT. 0.
C C             IER = 2 IF AN IEND OR IADJ ENTRY IS OUT OF
C C                     RANGE.
C C             IER = 3 IF THE TRIANGULATION PARAMETERS (NB,
C C                     NT, AND NA) ARE INCONSISTENT.
C C             IER = 4 IF A TRIANGLE CONTAINS A NODE INTERIOR
C C                     TO ITS CIRCUMCIRCLE.
C C             THE ERROR SITUATIONS ARE TESTED IN THE ORDER
C C               DEFINED BY THE (POSITIVE) IER VALUES.
C C
C C MODULE REQUIRED BY TRMTST - CIRCUM
C C
C C***********************************************************
C C
C       LOGICAL RITE
C C
C C SET LOCAL VARIABLES, TEST FOR ERRORS IN INPUT, AND
C C   INITIALIZE COUNTS.
C C
C       NN = N
C       RTOL = TOL
C       RITE = LUN .GE. 1  .AND.  LUN .LE. 99
C       IF (NN .LT. 3  .OR.  RTOL .LT. 0.) GO TO 5
C       NB = 0
C       NT = 0
C       NULL = 0
C       NFAIL = 0
C C
C C LOOP ON TRIANGLES (N1,N2,N3) SUCH THAT N2 AND N3 INDEX
C C   ADJACENT NEIGHBORS OF N1 AND ARE BOTH LARGER THAN N1
C C   (TRIANGLES ARE ASSOCIATED WITH THEIR SMALLEST INDEX).
C C
C       INDF = 1
C       DO 4 N1 = 1,NN
C         INDL = IEND(N1)
C         IF (INDL .LT. INDF+2) GO TO 6
C         IF (IADJ(INDL) .EQ. 0) NB = NB + 1
C C
C C   LOOP ON NEIGHBORS OF N1
C C
C         DO 3 INDX = INDF,INDL
C           N2 = IADJ(INDX)
C           IF (N2 .LT. 0  .OR.  N2 .GT. NN  .OR.
C      .        (INDX .LT. INDL  .AND.  N2 .EQ. 0)) GO TO 7
C           IF (INDX .LT. INDL) N3 = IADJ(INDX+1)
C           IF (INDX .EQ. INDL) N3 = IADJ(INDF)
C           IF (N2 .LT. N1  .OR.  N3 .LT. N1) GO TO 3
C           NT = NT + 1
C C
C C   COMPUTE THE COORDINATES OF THE CIRCUMCENTER OF
C C     (N1,N2,N3).
C C
C           CALL CIRCUM (X(N1),X(N2),X(N3),Y(N1),Y(N2),Y(N3),
C      .                 CX,CY,IERR)
C           IF (IERR .NE. 0) NULL = NULL + 1
C           IF (IERR .NE. 0) GO TO 3
C C
C C   TEST FOR NODES WITHIN THE CIRCUMCIRCLE.
C C
C           R = (CX-X(N1))**2 + (CY-Y(N1))**2 - RTOL
C           IF (R .LE. 0.) GO TO 3
C           DO 1 I = 1,NN
C             IF (I .EQ. N1  .OR.  I .EQ. N2  .OR.
C      .          I .EQ. N3) GO TO 1
C             IF ((CX-X(I))**2 + (CY-Y(I))**2 .LT. R) GO TO 2
C     1       CONTINUE
C           GO TO 3
C C
C C   NODE I IS INTERIOR TO THE CIRCUMCIRCLE OF (N1,N2,N3).
C C
C     2     NFAIL = NFAIL + 1
C     3     CONTINUE
C     4   INDF = INDL + 1
C C
C C CHECK PARAMETERS FOR CONSISTENCY AND TEST FOR NFAIL = 0.
C C
C       NA = (IEND(NN)-NB)/2
C       IF (NB .LT. 3  .OR.  NT .NE. 2*NN-NB-2  .OR.
C      .    NA .NE. 3*NN-NB-3) GO TO 8
C       IF (NFAIL .NE. 0) GO TO 9
C C
C C NO ERRORS WERE ENCOUNTERED.
C C
C       IER = 0
C       IF (NULL .EQ. 0) RETURN
C       IER = -1
C       IF (RITE) WRITE (LUN,100) NULL
C   100 FORMAT (1H ,10HTRMTST -- ,I5,16H NULL TRIANGLES ,
C      .        11HARE PRESENT/1H ,10X,16H(NULL TRIANGLES ,
C      .        32HON THE BOUNDARY ARE UNAVOIDABLE)//)
C       RETURN
C C
C C N OR TOL IS OUT OF RANGE.
C C
C     5 IER = 1
C       IF (RITE) WRITE (LUN,110) N, RTOL
C   110 FORMAT (1H ,33HTRMTST -- INVALID INPUT PARAMETER/
C      .        1H ,10X,4HN = ,I5,8H, TOL = ,E8.1)
C       RETURN
C C
C C IEND ENTRY OUT OF RANGE
C C
C     6 IER = 2
C       IF (RITE) WRITE (LUN,120) N1
C   120 FORMAT (1H ,15HTRMTST -- NODE ,I5,
C      .        26H HAS LESS THAN 3 NEIGHBORS)
C       RETURN
C C
C C IADJ ENTRY OUT OF RANGE
C C
C     7 IER = 2
C       IF (RITE) WRITE (LUN,130) INDX
C   130 FORMAT (1H ,33HTRMTST -- IADJ(K) IS NOT A VALID ,
C      .        14HINDEX FOR K = ,I5)
C       RETURN
C C
C C INCONSISTENT TRIANGULATION PARAMETERS
C C
C     8 IER = 3
C       IF (RITE) WRITE (LUN,140) NB, NT, NA
C   140 FORMAT (1H ,33HTRMTST -- INCONSISTENT PARAMETERS/
C      .        1H ,10X,I5,15H BOUNDARY NODES,3X,I5,
C      .        10H TRIANGLES,3X,I5,5H ARCS)
C       RETURN
C C
C C CIRCUMCIRCLE TEST FAILURE
C C
C     9 IER = 4
C       IF (RITE) WRITE (LUN,150) NFAIL
C   150 FORMAT (1H ,10HTRMTST -- ,I5,15H CIRCUMCIRCLES ,
C      .        32HCONTAIN NODES IN THEIR INTERIORS)
C       RETURN
C       END
C C
C C
